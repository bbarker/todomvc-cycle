...
function amendStateWithChildren(DOMSource: DOMSource<any>) {
  return function (todosData: TodosData): TodosData {
    return Object.assign(
      ...
      todosData,
      {
        ...
        list: todosData.list.map<TodosListItem>(data => {
          ...
          return Object.assign(
            ...
            data,
            {
              ...
              todoItem: {
                DOM: todoItem.DOM,
                action$: todoItem.action$.map(ev =>
                  Object.assign({}, ev, { id: data.id }))
              }
            });
        }),
      });
...
function TaskList(sources: Sources): Sinks {
  ...
  // let proxyItemAction$ = xs.create<TaskAction>();
  ...
  //   .map(state => xs.merge(...state.list.map(i => i.todoItem!.action$)))
  //   .flatten();
...
//TODO: Note I removed Cycle.js Collection while copying over TypeScript version